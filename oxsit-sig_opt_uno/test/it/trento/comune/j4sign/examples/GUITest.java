 ...
 */
/*
 * $Header: /cvsroot/j4sign/j4sign/src/java/core/it/trento/comune/j4sign/examples/GUITest.java,v 1.5 2008/05/27 21:33:21 resoli Exp $
 * $Revision: 1.5 $
 * $Date: 2008/05/27 21:33:21 $
 */
...
public class GUITest extends JFrame implements java.awt.event.ActionListener {
	...
	private DigestSignTask task = null;
	...
	private boolean makeDigestOnToken = true;
	...
	public GUITest(String title, String aDebug, String aCryptoki)
		...
		/*
		 * //Create a new sign task. task = new DigestSignTask(getCryptokiLib(),
		 * getSignerLabel(), log); //Create a timer. timer = new
		 * Timer(ONE_SECOND, new java.awt.event.ActionListener() { public void
		 * actionPerformed(java.awt.event.ActionEvent evt) {
		 * setStatus(task.getCurrent(), task.getMessage()); if (task.done()) {
		 * timer.stop(); progressBar.setValue(progressBar.getMinimum()); if
		 * (task.getCurrent() == DigestSignTask.SIGN_DONE) {
		 * Toolkit.getDefaultToolkit().beep();
		 * setEncryptedDigest(task.getEncryptedDigest());
		 * returnEncryptedDigestToForm(); setCertificate(task.getCertificate());
		 * returnCertificateToForm(); if (getSubmitAfterSigning()) {
		 * submitForm(); } } enableControls(true); } } });
		 */
		//dataArea.setText(decode(getEncodedData()));
	}
		...
		try {
			...
			if (e.getSource() == pwd) {
				...
				if (detectCardAndCriptoki()) {
					...
					task = new DigestSignTask(getCryptokiLib(),
							getSignerLabel(), log);
					//Create a timer.
					...
					timer = new Timer(ONE_SECOND,
							new java.awt.event.ActionListener() {
								...
								public void actionPerformed(
									...
									setStatus(task.getCurrent(), task
											.getMessage());
									if (task.done()) {
										timer.stop();
										...
										if (task.getCurrent() == DigestSignTask.SIGN_DONE) {
											...
											try {
												...
												log
														.println("Error closing signature process:\n"
																+ e);
											}
					...
					openSignature(CMSSignedDataGenerator.DIGEST_SHA1,
							CMSSignedDataGenerator.ENCRYPTION_RSA, this.makeDigestOnToken);
	...
	public void sign() {
		...
		if (!this.makeDigestOnToken && getEncodedDigest() == null)
			...
			long mechanism = -1L;
			if (CMSSignedDataGenerator.ENCRYPTION_RSA.equals(this.encAlg))
				if (this.makeDigestOnToken) {
					if (CMSSignedDataGenerator.DIGEST_MD5
							.equals(this.digestAlg))
			...
			if (mechanism == -1L)
				...
				task.setMechanism(mechanism);
				task.setPassword(pwd.getPassword());
				task.go();
				timer.start();
			}
		}
