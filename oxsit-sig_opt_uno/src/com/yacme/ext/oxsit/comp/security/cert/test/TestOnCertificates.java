...
package com.yacme.ext.oxsit.comp.security.cert.test;
	...
	private class CertInfo {
		...
		public String printCert() {
			...
			m_aLogger.info("Subject Public Signature Algorithm: "
					+ ((xc509.getSubjectPublicKeyInfo().getAlgorithmId().getObjectId()
							.equals(X509CertificateStructure.rsaEncryption)) ? "pkcs-1 rsaEncryption" : oi.getId()));
			...
			m_aLogger
					.info("Signature Algorithm: "
							+ ((xc509.getSignatureAlgorithm().getObjectId().equals(X509CertificateStructure.sha1WithRSAEncryption)) ? "pkcs-1 sha1WithRSAEncryption"
									: oi.getId()));
	...
	public void testMethod() {
		...
		while (it.hasNext()) {
			...
			if (ci != null) {
				...
				m_aLogger.log("\tDescription:\t" + ci.m_sDescription);
				m_aLogger.log("\tManufacturer:\t" + ci.m_sManufacturer);
				...
				if (availableToken != null) {
					for (int i = 0; i < availableToken.length; i++) {
						...
						Collection<?> certsOnToken = rt.getCertsOnToken(i);
