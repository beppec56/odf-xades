...
/**
 * Class for securely read characters from the console without showing it.<br>
 * Note that Java does not support this feature natively; the code originally
 * appeared on a web forum: {<a href="http://forum.java.sun.com/thread.jspa?forumID=9&threadID=490728">
 * http://forum.java.sun.com/thread.jspa?forumID=9&threadID=490728</a>) 
 */
public class PasswordMasker extends Object {

    public static void main(String[] args) throws Exception {
        System.out.println(PasswordMasker
                .readConsoleSecure("Enter password: "));
    }
...
*/
    public static final char[] readConsoleSecure(String prompt)
            throws IOException, InterruptedException {
        System.out.println();

        /*
         * start a separate thread which will mask out all characters typed on
         ...
         * input) while the user is in the middle of typing the password. This
         * may be achieved by trying to read just the first byte from System.in,
         * since reading from System.in blocks until it detects that an enter
         * has been pressed. Wrap System.in with a PushbackInputStream because
         * this byte will be unread below.
         * 
         * Problem: Dependent on the operating system or java vm(?) there is a
         * maximum number of bytes, which is buffered before enter is pressed.
         * E.g. on Solaris 8.0, JDK 1.4.1 exactly 256 bytes are buffered. That
         * means that no following return will be recognized to release the
         * current Thread, then.
         */
         ...
         * (e.g. the user pressed enter, or some error occurred?) signal
         * threadMasking to stop and wait till it is dead:
         */
        masker.stop();
        threadMasking.join();
        ...
        if (b == -1) {
            throw new IOException(
                    "end-of-file was detected in System.in without any "
                            + "data being read");
        }
	...
	*/
    protected static final char[] readLineSecure(PushbackInputStream in)
        ...
        try {
            ...
            while (loop) {
                ...
                case '\r':
                    ...
                    if ((c2 != '\n') && (c2 != -1)) {
                        /*
                         * guarantees that mac & dos line end sequences are
                         * completely read thru but not beyond
                         */
                        in.unread(c2);
                    }
                    loop = false;
	...
	*/
    protected static final char[] checkBuffer(char[] buffer, int offset)
        ...
        if (offset < buffer.length) {
            ...
            try {
                char[] bufferNew = new char[offset + buffer.length];
     ...
     * masking.
     * 
     * @see <a href="http://java.sun.com/features/2002/09/pword_mask.html"
     *      >Password masking in console </a>
     */
