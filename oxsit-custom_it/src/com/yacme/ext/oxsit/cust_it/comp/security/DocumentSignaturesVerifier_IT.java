...
//				}
			...
			try {
				if (xThePackage.isStreamElement(aElements[i])) {
					...
					try {
						...
						m_aLogger.debug("element: "+_rootElement+aElements[i]);
					...
					try {
						...
						m_aLogger.debug("recurse into element: "+_rootElement+aElements[i]);							
						fillElementList(xSubStore, _List, _rootElement+aElements[i]+"/", _bRecurse);
						xSubStore.dispose();
					}
					catch (IOException e) {
							m_aLogger.debug("fillElementList", "the substorage "+aElements[i]+" might be locked, get the last committed version of it");
							   try {
	...
	private Vector<ODFPackageItem> makeTheElementList(Object _othePackage, XStorage _xStorage) {
		...
		if (_xStorage == null) {
			...
			m_aLogger.debug("makeTheElementList", "use the URL storage");
			Utilities.showInterfaces(this, xThePackage);
		} else {
			xThePackage = _xStorage;
			m_aLogger.debug("makeTheElementList", "use the document storage");
		}
